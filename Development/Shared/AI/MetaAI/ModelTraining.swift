...
final class ModelTraining {
    ...
    private(set) var fewShotExamples: [FewShotExample] = []
    private(set) var promptTemplates: [PromptTemplate] = []
    ...
    func buildFewShotPrompt(
        for task: String,
        category: String,
        examples: [FewShotExample]? = nil
    ) -> String {
        let selectedExamples = examples ?? getFewShotExamples(for: category)
...
class FewShotExample: Identifiable {
    let id: UUID
    let category: String
    let input: String
    let output: String
