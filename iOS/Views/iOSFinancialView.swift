...
struct OverviewTabView: View {
    ...
    Menu {
      ForEach(financialManager.accounts) { account in
        Button {
          ...
          Label(
            account.accountName, systemImage: selectedAccount?.id == account.id ? "checkmark" : "")
        }
      }
    } label: {
      HStack {
  ...
  private func categoryBreakdown(for account: FinancialAccount) -> some View {
    let categorySpending = financialManager.getSpendingByCategory(
      for: account, period: selectedPeriod)
    ...
    VStack(alignment: .leading, spacing: 16) {
      ...
      if recommendations.isEmpty {
        ...
        ForEach(recommendations) { recommendation in
          RecommendationCard(
            recommendation: recommendation, currency: selectedAccount?.currency ?? "USD")
        }
      }
    }
  }
