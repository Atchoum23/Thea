...
final class AllProvidersTests: XCTestCase {
  ...
  func testAllProvidersSupportStreaming() {
    ...
    for provider in providers {
      XCTAssertTrue(
        ...
        "\(provider.metadata.name) should support streaming")
    }
  }
  ...
  func testGoogleHasLargestContext() {
    ...
    XCTAssertEqual(
      google.capabilities.maxContextTokens, 1_000_000, "Google should have 1M context window")
  }

  func testAnthropicHasLargestContext() {
    let anthropic = AnthropicProvider(apiKey: "test")
    XCTAssertEqual(
      anthropic.capabilities.maxContextTokens, 200000, "Anthropic should have 200K context")
  }
}
