...
final class EventTrimmingLogicTests: XCTestCase {
    ...
    func testTrimsWhenOverMax() {
        // Mirrors: if events.count > maxEventsInMemory { events.removeFirst(...) }
        let maxEventsInMemory = 1000
        var events = (0..<1050).map { AnalyticsEvent(name: "e\($0)", properties: [:], timestamp: Date()) }
        if events.count > maxEventsInMemory {
            events.removeFirst(events.count - maxEventsInMemory)
        }
        XCTAssertEqual(events.count, 1000)
        XCTAssertEqual(events.first?.name, "e50") // first 50 removed
    }

    func testNoTrimWhenUnderMax() {
        let maxEventsInMemory = 1000
        var events = (0..<500).map { AnalyticsEvent(name: "e\($0)", properties: [:], timestamp: Date()) }
        if events.count > maxEventsInMemory {
            events.removeFirst(events.count - maxEventsInMemory)
        }
    ...
    func testTrimAtExactMax() {
        let maxEventsInMemory = 1000
        var events = (0..<1000).map { AnalyticsEvent(name: "e\($0)", properties: [:], timestamp: Date()) }
        if events.count > maxEventsInMemory {
            events.removeFirst(events.count - maxEventsInMemory)
        }
        XCTAssertEqual(events.count, 1000) // no trim at exact boundary
    }
}
...
final class SessionTimeoutLogicTests: XCTestCase {
    ...
    func testSessionNotTimedOut() {
        ...
        let lastActivityTime = Date().addingTimeInterval(-10 * 60) // 10 min ago
        let isTimedOut = Date().timeIntervalSince(lastActivityTime) > sessionTimeout
...
final class BatchSyncLogicTests: XCTestCase {
    ...
    func testBatchSizeLimitsSync() {
        let batchSize = 50
        let events = (0..<200).map { AnalyticsEvent(name: "e\($0)", properties: [:], timestamp: Date()) }
    ...
    func testBatchSizeWithFewerEvents() {
        let batchSize = 50
        let events = (0..<10).map { AnalyticsEvent(name: "e\($0)", properties: [:], timestamp: Date()) }
        let eventsToSync = Array(events.prefix(batchSize))
    ...
    func testBatchRemovalMatchesBatchSize() {
        let batchSize = 50
        var events = (0..<200).map { AnalyticsEvent(name: "e\($0)", properties: [:], timestamp: Date()) }
...
final class UserPropertyIncrementTests: XCTestCase {
    ...
    func testIncrementFromZero() {
        // Mirrors: let current = (userProperties[key] as? Int) ?? 0
        var userProperties: [String: Any] = [:]
        let key = "feature_chat_count"
        let amount = 1
        let current = (userProperties[key] as? Int) ?? 0
        userProperties[key] = current + amount
