...
final class UnifiedDeviceAwareness {
    ...
    private init() {
        loadConfiguration()
        // Initialize network monitoring (uses modern NWPathMonitor)
    ...
    private func getScreenResolution() -> String {
        // Use modern scene-based API to avoid deprecated UIScreen.main
    ...
    /// Network path monitor for modern connectivity checking (replaces deprecated SCNetworkReachability)
    private static let networkPathMonitor = NWPathMonitor()
    private static var isNetworkAvailable = true
    private static var currentConnectionType = "Unknown"
    ...
    private static func setupNetworkMonitoring() {
        networkPathMonitor.pathUpdateHandler = { path in
            ...
            // Dispatch to main actor to update @MainActor-isolated properties
            Task { @MainActor in
    ...
    private func checkNetworkConnectivity() -> Bool {
        // Use modern NWPathMonitor instead of deprecated SCNetworkReachability
