...
//
// Types are extracted to DynamicResourceAllocatorTypes.swift for better organization
...
final class DynamicResourceAllocator {
    ...
    var adjustmentHistory: [ResourceAdjustment] = []
    ...
    func calculateOptimalAllocation() -> ResourceAllocation {
        ...
        // Step 0: Check for active scheduled modes
        let activeSchedule = getActiveScheduledMode()
        let effectiveMaxMemoryPercent: Double
        let effectiveAggressiveness: Configuration.AggressivenessLevel

        if let schedule = activeSchedule {
            effectiveMaxMemoryPercent = schedule.maxMemoryPercent
            effectiveAggressiveness = schedule.mode
            lastAdjustmentReason = "Scheduled mode active: \(schedule.mode.rawValue)"
        } else {
            effectiveMaxMemoryPercent = configuration.maxModelMemoryPercent
            effectiveAggressiveness = configuration.aggressivenessLevel
        }
        ...
        // Step 5: Apply aggressiveness level (from schedule or configuration)
        let aggressivenessMultiplier = effectiveAggressiveness.memoryMultiplier
        ...
        let maxAllocation = Double(effectiveAvailable) * effectiveMaxMemoryPercent * combinedMultiplier
    ...
    // MARK: - Scheduled Mode Management

    /// Get the currently active scheduled mode, if any
    func getActiveScheduledMode() -> ScheduledAllocationMode? {
        guard configuration.enableScheduledModes else { return nil }

        for schedule in configuration.scheduledModes where schedule.isActiveNow() {
            // Check additional conditions
            if schedule.requiresCharging && !systemMetrics.isCharging {
                continue
            }
            if schedule.requiresUserIdle && !isUserIdle() {
                continue
            }
            return schedule
        }
    ...
    /// Check if user is considered idle based on last activity
    private func isUserIdle() -> Bool {
        #if os(macOS)
        // Use IOKit to get idle time on macOS
        var iterator: io_iterator_t = 0
        ...
        if let property = propertyResult?.takeRetainedValue() as? NSNumber {
            // HIDIdleTime is in nanoseconds
            let idleSeconds = Double(property.int64Value) / 1_000_000_000.0
            return idleSeconds >= configuration.userIdleThresholdSeconds
        }
        return false
        #else
        // On iOS, assume not idle (would need more complex activity tracking)
        return false
        #endif
    }

    /// Add a new scheduled mode
    func addScheduledMode(_ mode: ScheduledAllocationMode) {
        configuration.scheduledModes.append(mode)
        saveConfiguration()
    }

    /// Remove a scheduled mode by ID
    func removeScheduledMode(id: UUID) {
        configuration.scheduledModes.removeAll { $0.id == id }
        saveConfiguration()
    }

    /// Update an existing scheduled mode
    func updateScheduledMode(_ mode: ScheduledAllocationMode) {
        if let index = configuration.scheduledModes.firstIndex(where: { $0.id == mode.id }) {
            configuration.scheduledModes[index] = mode
            saveConfiguration()
        }
    }
