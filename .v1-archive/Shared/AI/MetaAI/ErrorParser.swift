    ...
    public actor ErrorParser {
        ...
        private func genericSuggestion(for category: ErrorCategory) -> String? {
            switch category {
            case .sendable:
                "Add 'Sendable' conformance to the type"
            case .mainActor:
                "Add '@MainActor' attribute to the declaration"
            case .visibility:
                "Add 'public' modifier to make the symbol accessible"
            case .typeNotFound:
                "Check if the type is imported or defined"
            case .missingImport:
                "Add the required import statement"
            case .asyncAwait:
                "Add 'async' to the function and use 'await' when calling"
            case .missingInitializer:
                "Add missing parameters to the initializer"
            case .dataConcurrency:
                "Review concurrency isolation and use proper synchronization"
            case .unknown:
                nil
            }
        }
        ...
        public func analyzeErrors(_ errors: [ParsedError]) async -> ErrorStatistics {
            ...
            let totalWithFixes = errors.count { $0.suggestedFix != nil }
