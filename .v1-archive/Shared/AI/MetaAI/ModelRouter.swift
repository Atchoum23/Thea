...
/// Uses AI-powered adaptive routing that learns from performance metrics.
@MainActor
@Observable
public final class ModelRouter {
    ...
    private let logger = Logger(subsystem: "com.thea.metaai", category: "ModelRouter")
    #if os(macOS)
    private let localModelManager = LocalModelManager.shared
    #endif

    /// Enable AI-powered adaptive model selection (learns from performance)
    ...
    /// Uses AI-powered adaptive routing when enabled, learning from past performance
    public func selectModel(
        ...
        // 1. Try AI-powered adaptive selection (if enabled and we have enough data)
        if useAdaptiveRouting {
            let aiRecommendation = await AIIntelligence.shared.selectOptimalModel(
                for: classification.primaryType,
                constraints: constraints ?? ModelConstraints()
            )

            // If AI has high confidence based on learned patterns, use its recommendation
            if aiRecommendation.isLearned && aiRecommendation.confidence >= 0.75 {
                logger.info("Using AI-learned model selection: \(aiRecommendation.modelId) (confidence: \(String(format: "%.0f%%", aiRecommendation.confidence * 100)))")

                // Verify the model is available before returning
                if await isModelAvailable(aiRecommendation.modelId) {
                    let providerID = getProviderIDForModel(aiRecommendation.modelId)
                    return ModelSelection(
                        modelID: aiRecommendation.modelId,
                        providerID: providerID,
                        isLocal: aiRecommendation.modelId.hasPrefix("local-"),
                        estimatedCost: 0,
                        reasoning: "[AI-Adaptive] \(aiRecommendation.reasoning)"
                    )
                } else {
                    logger.warning("AI-recommended model \(aiRecommendation.modelId) not available, falling back to rule-based")
                }
            }
        }

        // 2. Get candidate models from routing rules (fallback path)
        ...
        // 3. Filter candidates by availability
        ...
        // 4. Apply local model preference logic
        ...
        if config.logModelRouting {
            logger.info("Selected \(selected.modelID) for \(classification.primaryType.displayName)")
            logger.debug("Reasoning: \(selected.reasoning)")
        }

        return selected
    }

    /// Record model performance for AI learning
    public func recordPerformance(
        ...
        AIIntelligence.shared.recordModelPerformance(
            modelId: modelID,
            taskType: taskType,
        ...
        logger.debug("Recorded performance: \(modelID) for \(taskType.rawValue) - success: \(wasSuccessful)")
    }

    private func getProviderIDForModel(_ modelID: String) -> String {
        if modelID.hasPrefix("local-") {
            return "local"
        } else if modelID.contains("openai") || modelID.contains("gpt") {
            return "openai"
    ...
    public func isModelAvailable(_ modelID: String) async -> Bool {
        ...
        if modelID.hasPrefix("local-") {
            ...
            #else
            return false // Local models not supported on this platform
            #endif
        }
    ...
    public func resolveLocalModel(_ pattern: String) -> String? {
        ...
        #else
        return nil // Local models not supported on this platform
        #endif
    }
