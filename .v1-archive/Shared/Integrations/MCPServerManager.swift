...
// MARK: - MCP Server Manager
    ...
    public class MCPServerManager: ObservableObject {
        ...
        private let logger = Logger(subsystem: "app.thea", category: "MCPServerManager")
        ...
        private init() {
            ...
            do {
                ...
                logger.error("Failed to create MCP config directory: \(error.localizedDescription)")
            }
        ...
        private func loadClaudeServers() -> [String: MCPServerConfig]? {
            ...
            do {
                ...
                logger.error("Failed to load Claude servers config: \(error.localizedDescription)")
                return nil
            }
        }

        private func loadTheaServers() -> [MCPInstalledServer]? {
            guard FileManager.default.fileExists(atPath: theaConfigPath.path) else {
                return nil
            }
            do {
                ...
                logger.error("Failed to load Thea servers config: \(error.localizedDescription)")
                return nil
            }
        }
        ...
        private func loadRegistryCache() -> [MCPRegistryServer]? {
            guard FileManager.default.fileExists(atPath: serverCachePath.path) else {
                return nil
            }
            do {
                let data = try Data(contentsOf: serverCachePath)
                let cache = try JSONDecoder().decode(MCPRegistryCache.self, from: data)
                guard Date().timeIntervalSince(cache.timestamp) < 86400 else { // 24 hour cache
                    return nil
                }
                return cache.servers
            } catch {
                logger.warning("Failed to load registry cache: \(error.localizedDescription)")
                return nil
            }
        }
        ...
        private func saveRegistryCache(_ servers: [MCPRegistryServer]) {
            ...
            do {
                ...
                logger.error("Failed to save registry cache: \(error.localizedDescription)")
            }
        }
        ...
        private func discoverTools(from serverId: String) async {
            ...
            do {
                ...
                do {
                    ...
                    logger.warning("Failed to decode tools list from server \(serverId): \(error.localizedDescription)")
                }
            } catch {
        ...
        public func exportToClaude(_ server: MCPInstalledServer) throws {
            ...
            if FileManager.default.fileExists(atPath: claudeConfigPath.path) {
                do {
                    ...
                    logger.warning("Failed to read existing Claude config, creating new: \(error.localizedDescription)")
