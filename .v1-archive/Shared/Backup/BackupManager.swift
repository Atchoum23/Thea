...
public final class BackupManager: ObservableObject {
                ...
                if autoBackupEnabled {
                    if let lastDate = lastBackupDate {
                        ...
                        do {
                            ...
                            logger.error("Initial auto backup failed: \(error.localizedDescription)")
                        }
                    }
                }
            }
    ...
    private func loadAvailableBackups() {
        ...
        do {
            ...
            logger.warning("Failed to read backup directory: \(error.localizedDescription)")
            return
        }
        ...
        if autoBackups.count > maxBackupCount {
            ...
            for backup in toDelete {
                do {
                    ...
                    logger.warning("Failed to cleanup old backup \(backup.name): \(error.localizedDescription)")
                }
            }
        }
    }
...
private struct ArchiveContainer: Codable {
