...
public actor ActivityLogger {
    ...
    private let logger = Logger(subsystem: "app.thea", category: "ActivityLogger")
        ...
        for (dayKey, dayEntries) in entriesByDay {
            ...
            if let data = try? Data(contentsOf: filePath) {
                ...
                do {
                    ...
                    logger.warning("Failed to decode existing log entries for \(dayKey): \(error.localizedDescription)")
                }
            }
            ...
            do {
                ...
                logger.error("Failed to save activity log for \(dayKey): \(error.localizedDescription)")
            }
        }
    }
    ...
    public func getEntries(for date: Date) async -> [ActivityLogEntry] {
        ...
        do {
            ...
            // File doesn't exist for this date - not an error
            return []
        }
        ...
        do {
            ...
            logger.warning("Failed to decode activity log for \(dayKey): \(error.localizedDescription)")
            return []
        }
    }
    ...
    public func cleanup() async {
        ...
        let files: [URL]
        do {
            files = try fileManager.contentsOfDirectory(at: logsDirectory, includingPropertiesForKeys: nil)
        } catch {
            logger.warning("Failed to list log files for cleanup: \(error.localizedDescription)")
            return
        }
        ...
        for file in files where file.pathExtension == "json" {
               ...
               fileDate < cutoffDate
            {
                do {
                    ...
                    logger.warning("Failed to delete old log file \(filename): \(error.localizedDescription)")
                }
            }
        }
    }
