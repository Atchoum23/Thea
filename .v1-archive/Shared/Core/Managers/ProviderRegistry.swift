...
final class ProviderRegistry {
    ...
    /// Tracks whether local providers have been initialized
    /// Used to prevent race conditions during provider access
    private(set) var localProvidersReady = false

    /// Continuation for awaiting local provider setup
    private var localProvidersContinuation: CheckedContinuation<Void, Never>?
    ...
    /// Wait for local providers to be ready (call before using local models)
    /// Based on 2025 best practice: "Synchronize provider initialization"
    func waitForLocalProviders() async {
        if localProvidersReady { return }

        await withCheckedContinuation { continuation in
            if localProvidersReady {
                continuation.resume()
            } else {
                localProvidersContinuation = continuation
            }
        }
    }
    ...
    private func setupLocalProviders() {
        ...
        // Use Task to avoid blocking but track completion (MainActor-bound since class is @MainActor)
        Task { [weak self] in
            await self?.refreshLocalProviders()
            self?.localProvidersReady = true

            // Resume any waiting continuations
            self?.localProvidersContinuation?.resume()
            self?.localProvidersContinuation = nil
        }
    }
    ...
    func refreshLocalProviders() async {
        ...
        #else
        debugLog("ℹ️ Local models not supported on this platform")
        #endif
    }
