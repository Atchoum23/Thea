    ...
    private func executeStep(_ step: ImplementationStep, task: EvolutionTask) async throws {
        ...
        case .createFile:
            guard let filePath = step.filePath else {
                throw EvolutionError.implementationFailed("Missing file path for create step: \(step.title)")
            }
            try await createFile(at: filePath, for: task)
        case .modifyFile:
            guard let filePath = step.filePath else {
                throw EvolutionError.implementationFailed("Missing file path for modify step: \(step.title)")
            }
            try await modifyFile(at: filePath, for: task)
        case .deleteFile:
            guard let filePath = step.filePath else {
                throw EvolutionError.implementationFailed("Missing file path for delete step: \(step.title)")
            }
        ...
        if result.success {
            ...
            if task.testsPassed {
                ...
                guard let outputPath = result.outputPath else {
                    throw EvolutionError.implementationFailed("Build succeeded but output path is missing")
                }
...
public enum EvolutionError: Error, LocalizedError {
    ...
    case implementationFailed(String)
    ...
    public var errorDescription: String? {
        ...
        case let .implementationFailed(reason): "Implementation failed: \(reason)"
        }
    }
}
