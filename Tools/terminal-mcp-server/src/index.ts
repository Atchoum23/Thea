 ...
 */
function truncateOutput(output: string, maxLength: number = MAX_OUTPUT_LENGTH): { text: string; truncated: boolean } {
 ...
 */
async function executeAppleScript(script: string): Promise<AppleScriptResult> {
  try {
...
Safety:
  ...
  async (params: z.infer<typeof ExecuteCommandSchema>) => {
    // Safety check
...
Examples:
  ...
  async (params: z.infer<typeof ExecuteAppleScriptSchema>) => {
    try {
...
Returns:
  ...
  async (params: z.infer<typeof ReadFileSchema>) => {
    try {
      ...
      const content = fs.readFileSync(resolvedPath, { encoding: params.encoding as BufferEncoding });
...
Returns:
  ...
  async (params: z.infer<typeof WriteFileSchema>) => {
    try {
      const expandedPath = params.path.replace(/^~/, os.homedir());
      const resolvedPath = path.resolve(expandedPath);
      ...
      fs.writeFileSync(resolvedPath, params.content, { encoding: params.encoding as BufferEncoding });
...
Returns:
  ...
  async (params: z.infer<typeof ListDirectorySchema>) => {
    try {
      const expandedPath = params.path.replace(/^~/, os.homedir());
      const resolvedPath = path.resolve(expandedPath);
...
Returns:
  ...
  async (params: { includeEnv: boolean }) => {
    const info = {
      ...getSystemInfo(),
      cwd: process.cwd(),
      ...(params.includeEnv ? {
