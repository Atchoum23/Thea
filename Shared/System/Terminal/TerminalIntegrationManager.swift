...
final class TerminalIntegrationManager: ObservableObject {
    ...
    enum ExecutionMode: String, CaseIterable, Codable {
        ...
        var description: String {
            switch self {
            case .direct: return "Execute commands directly (faster, captures output)"
            case .terminalApp: return "Execute in Terminal.app windows (interactive)"
            case .both: return "Direct execution with Terminal.app fallback"
            }
        }
    }
                ...
                do {
                    ...
                    if content != lastContent {
                        ...
                        if lastContent.isEmpty {
                            ...
                            newContent = String(content.dropFirst(lastContent.count))
                        } else {
                            newContent = content
                        }
    ...
    func loadCommandHistory() -> [TerminalCommand] {
        guard let data = try? Data(contentsOf: commandHistoryURL),
              let history = try? JSONDecoder().decode([TerminalCommand].self, from: data) else {
            return []
        }
        return history
    }
