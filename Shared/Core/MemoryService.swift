...
public actor MemoryService {
    ...
    public func getStatistics() async -> MemoryStatistics {
        ...
        let categoryDistribution = Dictionary(grouping: memories.compactMap(\.category), by: { $0 })
            .mapValues { $0.count }
...
public struct TheaMemory: Identifiable, Codable, Sendable {
    ...
    init?(from record: CKRecord) {
        guard let idString = record["id"] as? String,
              ...
              let type = TheaMemoryType(rawValue: typeString) else {
            return nil
        }
        ...
        self.source = MemorySource(rawValue: record["source"] as? String ?? "manual") ?? .manual
        self.confidence = record["confidence"] as? Double ?? 1.0
        self.createdAt = record["createdAt"] as? Date ?? Date()
        self.modifiedAt = record["modifiedAt"] as? Date ?? Date()
        self.lastAccessedAt = record["lastAccessedAt"] as? Date ?? Date()
        self.accessCount = record["accessCount"] as? Int ?? 0
    }
