    ...
    public final class ErrorKnowledgeBaseManager {
        ...
        public func getPreventionGuidance(forCode _: String) async -> [String] {
            guard let context = modelContext else { return [] }
            ...
            do {
                ...
                return filteredRecords.prefix(5).map(\.preventionRule)
            } catch {
                return []
            }
        }
        ...
        public func getSwiftDataErrorStats() async -> SwiftDataErrorStats {
            ...
            let avgSuccessRate = errors.isEmpty ? 0 : errors.map(\.successRate).reduce(0, +) / Float(errors.count)
