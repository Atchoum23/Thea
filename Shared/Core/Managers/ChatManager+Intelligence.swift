...
// Thea V4 — Orchestrator integration, branching, OCR, device context
//
// Extracted from ChatManager.swift for file size compliance.
// Prompt engineering delegated to TaskPromptBuilder (SRP).
...
extension ChatManager {
    ...
    // MARK: - Automatic Prompt Engineering (delegated to TaskPromptBuilder)

    /// Generates task-specific system prompt instructions based on the classified task type.
    /// Delegates to TaskPromptBuilder for clean separation of concerns.
    static func buildTaskSpecificPrompt(for taskType: TaskType) -> String {
        TaskPromptBuilder.buildPrompt(for: taskType)
    }
    ...
    static func extractPlanSteps(from text: String) -> [String] {
        TaskPromptBuilder.extractPlanSteps(from: text)
    }
}
