...
final class HealthInsightsViewModel {
    private let logger = Logger(subsystem: "ai.thea.app", category: "HealthInsightsViewModel")
    ...
    private func loadFromHealthKit(timeRange: TimeRange) async {
        ...
        for dayOffset in 0 ..< timeRange.days {
            ...
            do {
                ...
                logger.debug("Failed to fetch activity data for \(date): \(error.localizedDescription)")
            }
        }
        ...
        do {
            let hrRecords = try await healthKitService.fetchHeartRateData(for: hrRange)
