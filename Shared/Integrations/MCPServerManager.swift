...
public class MCPServerManager: ObservableObject {
    ...
    private func loadClaudeServers() -> [String: MCPServerConfig]? {
        guard FileManager.default.fileExists(atPath: claudeConfigPath.path),
              ...
              let config = try? JSONDecoder().decode(ClaudeDesktopConfig.self, from: data) else {
            return nil
        }
    ...
    private func loadTheaServers() -> [MCPInstalledServer]? {
        guard FileManager.default.fileExists(atPath: theaConfigPath.path),
              ...
              let servers = try? JSONDecoder().decode([MCPInstalledServer].self, from: data) else {
            return nil
        }
        return servers
    }
    ...
    private func loadRegistryCache() -> [MCPRegistryServer]? {
        guard FileManager.default.fileExists(atPath: serverCachePath.path),
              ...
              Date().timeIntervalSince(cache.timestamp) < 86400 else { // 24 hour cache
            return nil
        }
    ...
    public func install(_ registryServer: MCPRegistryServer) async throws {
        ...
        switch registryServer.installMethod {
        case .npm:
            server = try await installNpmServer(registryServer)
        case .pip:
            server = try await installPipServer(registryServer)
        case .binary:
            server = try await installBinaryServer(registryServer)
        case .docker:
            server = try await installDockerServer(registryServer)
        }
    ...
    public func executeTool(
        ...
        guard let tool = availableTools.first(where: { $0.id == toolId }),
              let pipes = serverPipes[tool.serverId] else {
...
struct MCPToolCallParams: Codable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        // Simplified - real implementation would encode arguments properly
    }
}
...
public enum MCPServerError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .installationFailed(let reason):
            return "Server installation failed: \(reason)"
        case .connectionFailed(let reason):
            return "Server connection failed: \(reason)"
        }
    }
}
