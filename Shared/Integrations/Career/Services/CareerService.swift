...
public actor CareerService: CareerServiceProtocol, SkillTrackingProtocol, CareerCareerReflectionServiceProtocol, GrowthRecommendationProtocol {
    private let logger = Logger(subsystem: "ai.thea.app", category: "CareerService")
    // MARK: - Properties
    ...
    public func generateRecommendations(
        ...
        do {
            let wellbeingRecs = try await generateWellbeingRecommendations(reflections: reflections)
            newRecommendations.append(contentsOf: wellbeingRecs)
        } catch {
            logger.error("Failed to generate wellbeing recommendations: \(error)")
        }
