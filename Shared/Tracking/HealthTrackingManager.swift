...
import SwiftData
  ...
  final class HealthTrackingManager {
    ...
    func fetchDailySnapshot(for date: Date) async -> HealthSnapshot {
      ...
      let (stepsValue, caloriesValue, hrStats, sleepValue, workoutValue) = await (
    ...
    private func fetchHeartRateStatistics(from start: Date, to end: Date) async -> (
      ...
      return await withCheckedContinuation { continuation in
        let query = HKStatisticsQuery(
          ...
          let average = result?.averageQuantity()?.doubleValue(
            for: HKUnit.count().unitDivided(by: .minute()))
          let min = result?.minimumQuantity()?.doubleValue(
            for: HKUnit.count().unitDivided(by: .minute()))
          let max = result?.maximumQuantity()?.doubleValue(
            for: HKUnit.count().unitDivided(by: .minute()))
