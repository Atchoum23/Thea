...
import SwiftData
  ...
  final class ScreenTimeTracker {
    ...
    func startTracking() {
      ...
      timer = Timer.scheduledTimer(withTimeInterval: config.screenTimeCheckInterval, repeats: true)
      { [weak self] _ in
        Task { @MainActor in
    ...
    func stopTracking() {
      ...
      // Save final session
      if let currentApp = currentApp, let startTime = appStartTime {
    ...
    private func categorizeApp(_ appName: String) -> AppUsage.AppCategory {
      ...
      // Development tools
      if name.contains("xcode") || name.contains("terminal") || name.contains("code")
      ...
      // Communication
      if name.contains("mail") || name.contains("messages") || name.contains("slack")
      ...
      // Social media
      if name.contains("twitter") || name.contains("facebook") || name.contains("instagram")
      ...
      // Entertainment
      if name.contains("spotify") || name.contains("music") || name.contains("youtube")
      ...
      // Productivity
      if name.contains("safari") || name.contains("chrome") || name.contains("notes")
        || name.contains("pages") || name.contains("numbers")
      {
        return .productive
      }
