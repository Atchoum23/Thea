...
public actor AutomationEngine {
    ...
    public func classifyAction(_ action: ActionType) -> ConsequenceLevel {
        ...
        case .click, .scroll:
            return .moderate
        case .type(let text) where text.contains("sudo") || text.contains("rm -rf"):
            return .critical
        case .type:
            return .moderate
        case .keyPress(let key, _) where key == "Delete" || key == "Backspace":
            return .high
        case .keyPress:
            return .moderate
        }
...
public enum AutomationError: Error, Sendable, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .platformNotSupported:
            return "This automation feature is only supported on macOS"
