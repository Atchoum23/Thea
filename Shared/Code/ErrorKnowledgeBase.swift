...
final class ErrorKnowledgeBase {
  ...
  func recordError(
    ...
    if let existingError = similar.first(where: { $0.errorPattern == errorPattern }) {
      ...
      if !fix.isEmpty {
        let newSuccessRate =
          (existingError.successRate * Float(existingError.occurrenceCount - 1) + 1.0)
          / Float(existingError.occurrenceCount)
  ...
  private func categorizeErrorByMessage(_ message: String) -> String {
    ...
    if lowercased.contains("concurrency") || lowercased.contains("sendable")
      || lowercased.contains("actor")
    {
      return "concurrency"
    }
  ...
  func getErrorStats() async -> ErrorStats {
    ...
    do {
      ...
      let avgSuccessRate =
        errors.isEmpty ? 0 : errors.map { $0.successRate }.reduce(0, +) / Float(errors.count)
