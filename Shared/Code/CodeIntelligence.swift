...
final class CodeIntelligence {
  ...
  func getCodeCompletions(file: URL, position: Int, context: String) async throws
    -> [CodeCompletion]
  {
    ...
    let stream = try await provider.chat(
      messages: [message], model: codeConfig.codeCompletionModel, stream: true)
  ...
  func explainCode(_ code: String, language: ProgrammingLanguage) async throws -> String {
    ...
    var response = ""
    let stream = try await provider.chat(
      messages: [message], model: codeConfig.codeExplanationModel, stream: true)
  ...
  func reviewCode(_ code: String, language: ProgrammingLanguage) async throws -> CodeReview {
    ...
    var response = ""
    let stream = try await provider.chat(
      messages: [message], model: codeConfig.codeReviewModel, stream: true)
