...
final class SwiftBestPracticesLibrary: @unchecked Sendable {
  ...
  private static func createBestPractices() -> [SwiftBestPractice] {
    ...
    // MARK: - Concurrency Practices

    practices.append(
      SwiftBestPractice(
        category: .concurrency,
        pattern: "Always use @MainActor for UI code",
        explanation:
          "UI updates must happen on the main thread. Mark view models, SwiftUI views, and UI-related properties with @MainActor.",
        goodExample: "@MainActor class ViewModel: ObservableObject { @Published var data: String }",
        badExample: "class ViewModel: ObservableObject { @Published var data: String }",
        priority: .critical
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .concurrency,
        pattern: "Make types Sendable for cross-actor usage",
        explanation:
          "Types passed across actor boundaries must be Sendable. Use actors, value types, or explicit conformance.",
        goodExample: "struct User: Sendable { let id: UUID; let name: String }",
        badExample: "class User { var id: UUID; var name: String }",
        priority: .critical
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .concurrency,
        pattern: "Use async/await instead of completion handlers",
        explanation: "Modern Swift prefers async/await for better readability and error handling.",
        goodExample:
          "func fetchData() async throws -> Data { try await URLSession.shared.data(from: url).0 }",
        badExample: "func fetchData(completion: @escaping (Result<Data, Error>) -> Void) { }",
    ...
    practices.append(
      SwiftBestPractice(
        category: .concurrency,
        pattern: "Avoid @unchecked Sendable unless absolutely necessary",
        ...
        badExample: "class UnsafeWrapper: @unchecked Sendable { var value: Int }",
        priority: .critical
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .concurrency,
        pattern: "Use actor for mutable shared state",
        explanation:
          "Actors provide automatic synchronization for mutable state accessed from multiple contexts.",
        goodExample: "actor DataCache { private var cache: [String: Data] = [:] }",
        badExample: "class DataCache { private var cache: [String: Data] = [:] }",
        priority: .critical
      ))

    // MARK: - SwiftUI Practices

    practices.append(
      SwiftBestPractice(
        category: .swiftUI,
        pattern: "Use @Observable instead of ObservableObject",
        explanation:
          "In iOS 17+/macOS 14+, @Observable provides better performance and simpler syntax.",
        goodExample: "@Observable class ViewModel { var count: Int = 0 }",
        badExample: "class ViewModel: ObservableObject { @Published var count: Int = 0 }",
        priority: .recommended
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .swiftUI,
        pattern: "Use @Environment for dependency injection",
        ...
        badExample: "init(modelContext: ModelContext) { self.modelContext = modelContext }",
        priority: .recommended
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .swiftUI,
        pattern: "Extract complex views into separate components",
        ...
        badExample: "var body: some View { VStack { /* 100 lines of code */ } }",
        priority: .recommended
      ))

    // MARK: - Optional Practices

    practices.append(
      SwiftBestPractice(
        category: .optionals,
        pattern: "Use guard let for early returns",
        explanation:
          "Guard statements keep the happy path un-indented and make code more readable.",
        goodExample: "guard let user = user else { return }; print(user.name)",
        badExample: "if let user = user { print(user.name) }",
        priority: .recommended
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .optionals,
        pattern: "Avoid force unwrapping (!)",
        explanation:
          "Force unwrapping crashes at runtime. Use optional binding or nil coalescing instead.",
        goodExample: "let value = optional ?? defaultValue",
        badExample: "let value = optional!",
        priority: .critical
      ))
    ...
    practices.append(
      SwiftBestPractice(
        ...
        badExample: "let count = user!.posts!.count",
        priority: .recommended
      ))

    // MARK: - Protocol Practices

    practices.append(
      SwiftBestPractice(
        category: .protocols,
        pattern: "Prefer protocol-oriented design",
        ...
        badExample: "class DataProvider { func fetchData() async throws -> Data }",
        priority: .recommended
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .protocols,
        pattern: "Use protocol extensions for default implementations",
        explanation: "Provide default behavior in protocol extensions.",
        goodExample:
          "extension DataProvider { func fetchData() async throws -> Data { /*default*/ } }",
        badExample: "Make every conforming type implement the same boilerplate",
        priority: .recommended
      ))

    // MARK: - Error Handling Practices

    practices.append(
      SwiftBestPractice(
        category: .errorHandling,
        pattern: "Define custom error types",
        ...
        badExample: "throw NSError(domain: \"Error\", code: -1, userInfo: nil)",
        priority: .recommended
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .errorHandling,
        pattern: "Handle errors at appropriate levels",
    ...
    practices.append(
      SwiftBestPractice(
        category: .errorHandling,
        pattern: "Use Result type for async operations without async/await",
        ...
        badExample: "func loadData(completion: @escaping (Data?, Error?) -> Void)",
        priority: .recommended
      ))

    // MARK: - Memory Management Practices

    practices.append(
      SwiftBestPractice(
        category: .memoryManagement,
        pattern: "Use [weak self] in closures to avoid retain cycles",
        ...
        badExample: "task = Task { await self.loadData() }",
        priority: .critical
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .memoryManagement,
        pattern: "Understand value vs reference semantics",
        ...
        badExample: "class ImmutableData { let values: [Int] } // Should be struct",
        priority: .recommended
      ))

    // MARK: - Generics Practices

    practices.append(
      SwiftBestPractice(
        category: .generics,
        pattern: "Use generics to avoid code duplication",
        explanation: "Generic code is reusable and type-safe.",
        goodExample: "func transform<T>(_ value: T, with: (T) -> T) -> T",
        badExample: "func transformInt(_ value: Int, with: (Int) -> Int) -> Int",
        priority: .recommended
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .generics,
        pattern: "Constrain generics appropriately",
        ...
        badExample: "func sort<T>(_ items: [T]) -> [T]",
        priority: .recommended
      ))

    // MARK: - Modern Swift Practices

    practices.append(
      SwiftBestPractice(
        category: .modernSwift,
        pattern: "Use property wrappers for common patterns",
        ...
        badExample: "var count: Int { get { UserDefaults.standard.integer(...) } }",
        priority: .stylistic
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .modernSwift,
        pattern: "Prefer let over var",
        explanation: "Immutability prevents bugs and makes code easier to reason about.",
        goodExample: "let constant = 42",
        badExample: "var constant = 42 // Never modified",
        priority: .recommended
      ))

    // MARK: - Naming Practices

    practices.append(
      SwiftBestPractice(
        category: .naming,
        pattern: "Use clear, descriptive names",
        ...
        badExample: "func getUP(id: UUID) async throws -> UP",
        priority: .critical
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .naming,
        pattern: "Boolean properties should read as assertions",
        explanation: "Use is/has/can prefixes for boolean properties.",
        goodExample: "var isLoading: Bool; var hasData: Bool",
        badExample: "var loading: Bool; var data: Bool",
        priority: .recommended
      ))

    // MARK: - Architecture Practices

    practices.append(
      SwiftBestPractice(
        category: .architecture,
        pattern: "Separate concerns with MVVM or similar patterns",
        explanation: "Keep views, business logic, and data separate.",
        goodExample: "struct ContentView: View { @State var viewModel = ViewModel() }",
        badExample: "All logic, UI, and data access in one massive view",
        priority: .recommended
      ))
    ...
    practices.append(
      SwiftBestPractice(
        ...
        badExample: "let apiClient = APIClient.shared",
        priority: .recommended
      ))

    // MARK: - Performance Practices

    practices.append(
      SwiftBestPractice(
        category: .performance,
        pattern: "Use lazy var for expensive computations",
        explanation: "Defer expensive work until actually needed.",
        goodExample: "lazy var expensiveData: Data = { computeExpensiveData() }()",
        badExample: "var expensiveData: Data = computeExpensiveData()",
        priority: .recommended
      ))
    ...
    practices.append(
      SwiftBestPractice(
        category: .performance,
        pattern: "Avoid creating excessive temporary objects",
        explanation: "Reuse objects and use value types appropriately.",
        goodExample: "let result = items.reduce(0, +)",
        badExample: "var result = 0; for item in items { result += item }",
        priority: .stylistic
      ))

    // MARK: - Security Practices

    practices.append(
      SwiftBestPractice(
        category: .security,
        pattern: "Never hardcode secrets or API keys",
        ...
        badExample: "let apiKey = \"sk_live_1234567890abcdef\"",
        priority: .critical
      ))
    ...
    practices.append(
      SwiftBestPractice(
        ...
        badExample: "let email = userInput // Used directly",
        priority: .critical
      ))
