...
final class DeepAgentEngine {
  ...
  func executeTask(_ instruction: String, context: TaskContext = TaskContext()) async throws
    -> TaskResult
  {
    isProcessing = true
  ...
  private func decomposeTask(_ instruction: String, type: TaskType, context: TaskContext)
    async throws -> [Subtask]
  {
    ...
    case .appDevelopment:
      return [
        Subtask(
          step: 1, description: "Design application architecture and data models", dependencies: []),
    ...
    case .research:
      return [
        ...
        Subtask(
          step: 2, description: "Gather information from multiple sources", dependencies: [1]),
        Subtask(step: 3, description: "Analyze and synthesize findings", dependencies: [2]),
        Subtask(
          step: 4, description: "Generate structured report with citations", dependencies: [3]),
      ]
  ...
  private func dynamicDecomposition(_ instruction: String, context: TaskContext) async throws
    -> [Subtask]
  {
    // Use reasoning engine for custom decomposition
    let taskType = classifyTask(instruction)
    let subtaskDescriptions = try await reasoningEngine.decomposeTask(
      instruction, taskType: taskType, maxSteps: 10)
  ...
  private func executeWithVerification(plan: DeepTaskPlan, context: TaskContext) async throws
    -> [SubtaskResult]
  {
    ...
    for subtaskWithTools in plan.subtasks {
      ...
      let result = try await executeSubtask(
        subtaskWithTools, context: context, previousResults: results)
  ...
  private func executeSubtask(
    _ subtaskWithTools: SubtaskWithTools, context: TaskContext, previousResults: [SubtaskResult]
  ...
  private func verifyResult(_ result: SubtaskResult, subtask: Subtask) async throws
    -> VerificationResult
  {
    // Use reasoning engine to verify
    let verification = try await reasoningEngine.verifyOutput(
      result.output, expectedCriteria: [subtask.description])
  ...
  private func selfCorrect(
    subtask: SubtaskWithTools, failedResult: SubtaskResult, verification: VerificationResult,
    context: TaskContext
  ) async throws -> SubtaskResult {
  ...
  private func synthesizeResults(_ results: [SubtaskResult], originalInstruction: String)
...
protocol DeepTool: Sendable {
  ...
  func execute(input: String, context: TaskContext, previousResults: [SubtaskResult]) async throws
    -> String
}
