    ...
    public actor AICodeFixGenerator {
        ...
        public enum AIError: LocalizedError, Sendable {
            ...
            public var errorDescription: String? {
                ...
                case let .generationFailed(message):
                    "Fix generation failed: \(message)"
                case .invalidResponse:
                    "Received invalid response from AI provider"
                case let .providerError(message):
                    "Provider error: \(message)"
                case .rateLimited:
                    "Rate limited by AI provider. Please try again later."
                case .contextTooLarge:
                    "Code context too large for the model's context window."
                }
            }
        }
        ...
        private func isProviderConfigured(_ providerId: String) async -> Bool {
            ...
            case "anthropic", "openai", "openrouter", "groq", "google", "perplexity":
                ...
                await SecureStorage.shared.hasAPIKey(for: providerId)
            case "local":
