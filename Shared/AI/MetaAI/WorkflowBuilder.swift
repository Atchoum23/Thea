...
final class WorkflowBuilder {
  ...
  nonisolated private func executeAIInference(
    ...
    let provider = await ProviderRegistry.shared.getProvider(
      id: await SettingsManager.shared.defaultProvider)!
  ...
  nonisolated private func executeToolNode(
    ...
    guard let tool = await toolFramework.registeredTools.first(where: { $0.name == toolName })
    else {
...
class Workflow: Identifiable, Hashable, @unchecked Sendable {
  ...
  init(
    id: UUID, name: String, description: String, nodes: [WorkflowNode], edges: [WorkflowEdge],
    variables: [String: Any], isActive: Bool, createdAt: Date, modifiedAt: Date
  ) {
    self.id = id
    self.name = name
...
class WorkflowNode: Identifiable, @unchecked Sendable {
  ...
  init(
    id: UUID, type: WorkflowNodeType, position: CGPoint, config: [String: Any], inputs: [NodePort],
    outputs: [NodePort]
  ) {
    self.id = id
    self.type = type
...
class WorkflowExecution: Identifiable {
  ...
  init(
    id: UUID, workflowId: UUID, startTime: Date, status: ExecutionStatus, currentNode: UUID?,
    outputs: [UUID: [String: Any]], errors: [String]
  ) {
    self.id = id
