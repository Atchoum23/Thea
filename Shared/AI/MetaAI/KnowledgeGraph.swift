...
final class KnowledgeGraph {
    ...
    private func determineRelationType(
        ...
        let response = try await streamProviderResponse(provider: provider, prompt: prompt, model: config.knowledgeGraphModel)
    ...
    private func nameClusters() async throws {
        for i in 0..<clusters.count {
            ...
            let name = try await streamProviderResponse(provider: provider, prompt: prompt, model: config.knowledgeGraphModel)
    ...
    private func reasonWithKnowledge(query: String, knowledge: String) async throws -> String {
        ...
        return try await streamProviderResponse(provider: provider, prompt: prompt, model: config.knowledgeGraphModel)
    }
...
class KnowledgeNode: Identifiable, Hashable {
    ...
    init(id: UUID, content: String, type: NodeType, embedding: [Float], metadata: [String: String], createdAt: Date, importance: Float) {
        self.id = id
