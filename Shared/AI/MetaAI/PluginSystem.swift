...
final class PluginSystem {
  ...
  nonisolated func sendMessage(
    ...
    guard let sourcePlugin = await pluginIndex[sourcePluginId.uuidString],
      let targetPlugin = await pluginIndex[targetPluginId.uuidString]
    else {
      throw PluginError.pluginNotFound
    }
  ...
  private func requestPermissions(_ permissions: [PluginPermission]) async throws
    -> [PluginPermission]
  {
  ...
  private func validateManifest(_ manifest: PluginManifest) throws {
      ...
      && manifest.permissions.contains(.networkAccess)
    {
      throw PluginError.invalidManifest(
        "Cannot combine systemCommands and networkAccess permissions")
    }
  }
...
class Plugin: Identifiable, @unchecked Sendable {
  ...
  init(
    id: UUID, manifest: PluginManifest, isEnabled: Bool, grantedPermissions: [PluginPermission],
    installedAt: Date, lastExecuted: Date?
  ) {
    self.id = id
