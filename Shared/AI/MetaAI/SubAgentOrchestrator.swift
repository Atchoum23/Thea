...
final class SubAgentOrchestrator {
  ...
  enum AgentType: String, CaseIterable, Codable {
    ...
    var systemPrompt: String {
      ...
      case .researcher:
        return
          "You are a research specialist. Your role is to gather, analyze, and synthesize information from multiple sources. You provide comprehensive, well-sourced insights."
      case .coder:
        let bestPractices = SwiftBestPracticesLibrary.shared.getPracticesForContext("")
        let bestPracticesText = SwiftBestPracticesLibrary.shared.formatForPrompt(
          Array(bestPractices.prefix(10)))
      ...
      case .analyst:
        return
          "You are a data analyst. You examine data, identify patterns, and provide actionable insights. You excel at breaking down complex information."
      case .writer:
        return
          "You are a professional writer. You create clear, engaging, and well-structured content tailored to the audience and purpose."
      case .planner:
        return
          "You are a strategic planner. You break down complex goals into actionable steps, identify dependencies, and create efficient execution plans."
      case .critic:
        return
          "You are a constructive critic. You identify flaws, gaps, and areas for improvement in ideas, plans, and implementations."
      case .executor:
        return
          "You are an executor. You take plans and implement them step-by-step, ensuring each action is completed correctly."
      case .integrator:
        return
          "You are an integrator. You combine outputs from multiple agents into cohesive, unified results."
      case .validator:
        return
          "You are a validator. You verify that outputs meet requirements, are accurate, and align with objectives."
      case .optimizer:
        return
          "You are an optimizer. You improve existing solutions for efficiency, performance, and quality."
      }
    }
  ...
  private func planTask(_ task: String, context: [String: Any]) async throws -> TaskPlan {
    ...
    let planText = try await streamProviderResponse(
      provider: provider, prompt: prompt, model: config.plannerModel)
  ...
  private func executeSubTask(_ assignment: AgentAssignment) async throws -> SubTaskResult {
    ...
    let output = try await streamProviderResponse(
      provider: provider, prompt: prompt, model: "gpt-4o")
  ...
  private func executeCoderTaskWithValidation(_ assignment: AgentAssignment) async throws
    -> SubTaskResult
  {
    ...
    while attempts < maxAttempts {
      ...
      if attempts > 1 && !lastErrors.isEmpty {
        ...
        let preventionRules = await errorLearning.getPreventionGuidance(
          forCode: assignment.subTask.description)
        ...
        case .failure(let errors):
          ...
          if attempts >= maxAttempts {
            ...
            return SubTaskResult(
              ...
              agent: assignment.agent,
              output:
                "❌ Code validation failed after \(maxAttempts) attempts:\n\n\(errorSummary)\n\nLast generated code:\n```swift\n\(swiftCode)\n```",
              success: false,
              executionTime: Date().timeIntervalSince(startTime)
            )
          }
  ...
  private func integrateResults(_ results: [SubTaskResult]) async throws -> String {
    ...
    let allOutputs = results.map { "[\($0.agent.type.rawValue)]: \($0.output)" }.joined(
      separator: "\n\n")
