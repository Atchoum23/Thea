...
final class MemorySystem {
  ...
  func addShortTermMemory(content: String, type: MemoryType, metadata: [String: String] = [:])
    async throws -> Memory
  {
  ...
  func addEpisodicMemory(
    event: String, context: String, participants: [String] = [], metadata: [String: String] = [:]
  ) async throws -> EpisodicMemory {
  ...
  func addSemanticMemory(
    concept: String, definition: String, relatedConcepts: [String] = [],
    metadata: [String: String] = [:]
  ) async throws -> SemanticMemory {
  ...
  func addProceduralMemory(
    skill: String, steps: [String], conditions: [String] = [], metadata: [String: String] = [:]
  ) async throws -> ProceduralMemory {
  ...
  func retrieveRelevantMemories(for query: String, limit: Int? = nil, threshold: Float? = nil)
    async throws -> [Memory]
  {
  ...
  func retrieveEpisodicMemories(for query: String, limit: Int? = nil, threshold: Float? = nil)
    async throws -> [EpisodicMemory]
  {
    let queryEmbedding = try await generateEmbedding(for: query)
  ...
  func retrieveSemanticMemories(for concept: String, limit: Int? = nil, threshold: Float? = nil)
    async throws -> [SemanticMemory]
  {
  ...
  func retrieveProceduralMemories(for task: String, limit: Int? = nil, threshold: Float? = nil)
    async throws -> [ProceduralMemory]
  {
  ...
  func summarizeMemories(memories: [Memory]) async throws -> String {
    ...
    let stream = try await provider.chat(
      messages: [message], model: summarizationModel, stream: true)
  ...
  private func updateMemoryAccess(_ memory: Memory) {
    if let index = shortTermMemory.firstIndex(where: { $0.id == memory.id }) {
      ...
      longTermMemory[index].importance = min(
        longTermMemory[index].importance * config.accessImportanceBoost, 1.0)
    }
  }
  ...
  private func updateEpisodicAccess(_ memory: EpisodicMemory) {
    if let index = episodicMemory.firstIndex(where: { $0.id == memory.id }) {
      ...
      episodicMemory[index].importance = min(
        episodicMemory[index].importance * config.accessImportanceBoost, 1.0)
    }
  }
  ...
  private func updateSemanticAccess(_ memory: SemanticMemory) {
    if let index = semanticMemory.firstIndex(where: { $0.id == memory.id }) {
      ...
      semanticMemory[index].importance = min(
        semanticMemory[index].importance * config.accessImportanceBoost, 1.0)
    }
  }
  ...
  private func updateProceduralAccess(_ memory: ProceduralMemory) {
    if let index = proceduralMemory.firstIndex(where: { $0.id == memory.id }) {
      ...
      proceduralMemory[index].importance = min(
        proceduralMemory[index].importance * config.accessImportanceBoost, 1.0)
    }
  }
  ...
  private func startConsolidationTimer() async {
    let consolidationInterval = UInt64(
      AppConfiguration.shared.agentConfig.consolidationIntervalSeconds * 1_000_000_000)
