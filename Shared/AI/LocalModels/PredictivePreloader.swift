...
final class PredictivePreloader: @unchecked Sendable {
    ...
    private func loadPersistedState() {
        ...
        if let data = UserDefaults.standard.data(forKey: transitionMatrixKey) {
            do {
                ...
                logger.error("Failed to decode transition matrix: \(error.localizedDescription)")
            }
        }
        ...
        if let data = UserDefaults.standard.data(forKey: timeOfDayPatternsKey) {
            do {
                ...
                logger.error("Failed to decode time-of-day patterns: \(error.localizedDescription)")
            }
        }
        ...
        if let data = UserDefaults.standard.data(forKey: recentTasksKey) {
            do {
                recentTasks = try JSONDecoder().decode([TaskTypeTimestamp].self, from: data)
            } catch {
                logger.error("Failed to decode recent tasks: \(error.localizedDescription)")
            }
        }
    ...
    private func persistState() {
        ...
        do {
            let data = try JSONEncoder().encode(encodableMatrix)
            UserDefaults.standard.set(data, forKey: transitionMatrixKey)
        } catch {
            logger.error("Failed to encode transition matrix: \(error.localizedDescription)")
        }
        ...
        do {
            let data = try JSONEncoder().encode(encodableToD)
            UserDefaults.standard.set(data, forKey: timeOfDayPatternsKey)
        } catch {
            logger.error("Failed to encode time-of-day patterns: \(error.localizedDescription)")
        }

        // Save recent tasks
        do {
            let data = try JSONEncoder().encode(recentTasks)
            UserDefaults.standard.set(data, forKey: recentTasksKey)
        } catch {
            logger.error("Failed to encode recent tasks: \(error.localizedDescription)")
        }
    }
