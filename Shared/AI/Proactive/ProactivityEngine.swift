...
public final class ProactivityEngine: ObservableObject {
    ...
    private func detectContradiction(
        ...
        do {
            ...
            let originalVersions = versionPattern.matches(in: original, range: NSRange(original.startIndex..., in: original))
            let newVersions = versionPattern.matches(in: newLower, range: NSRange(newLower.startIndex..., in: newLower))
            ...
            logger.error("Failed to compile version regex: \(error.localizedDescription)")
        }
