...
final class PromptOptimizer {
  ...
  func optimizePrompt(
    ...
    if let template = await selectBestTemplate(for: agentType, taskInstruction: taskInstruction) {
      optimizedPrompt = interpolateTemplate(
        template.templateText, with: taskInstruction, context: context)
    } else {
  ...
  private func interpolateTemplate(
    _ template: String, with instruction: String, context: TaskContext
  ) -> String {
    var result = template
  ...
  private func formatContext(_ context: TaskContext) -> String {
    ...
    if !context.verificationIssues.isEmpty {
      contextParts.append(
        "Known issues to avoid: \(context.verificationIssues.joined(separator: ", "))")
    }
  ...
  private func getCodeFewShotExamples(for agentType: SubAgentOrchestrator.AgentType, limit: Int)
    async -> [CodeFewShotExample]
  {
    ...
    let descriptor = FetchDescriptor<CodeFewShotExample>(
      ...
      sortBy: [
        SortDescriptor(\.quality, order: .reverse), SortDescriptor(\.usageCount, order: .forward),
      ]
    )
  ...
  func recordOutcome(
    ...
    if let template = promptTemplate {
      ...
      template.averageConfidence = Float(
        (Double(template.averageConfidence) * Double(totalAttempts - 1) + confidence)
  ...
  func getOptimizationStats() async -> OptimizationStats {
    ...
    let avgSuccessRate =
      templates.isEmpty ? 0 : templates.map { $0.successRate }.reduce(0, +) / Float(templates.count)
