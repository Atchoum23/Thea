...
final class UserPreferenceModel {
  ...
  private func analyzeChanges(from original: String, to corrected: String) -> [(
  ...
  private func extractCharacteristics(from output: String, category: String) -> [(
    key: String, value: String
  )] {
  ...
  func getPreferenceStats() async -> PreferenceStats {
    ...
    do {
      ...
      let avgConfidence =
        preferences.isEmpty
        ? 0 : preferences.map { $0.confidence }.reduce(0, +) / Float(preferences.count)
