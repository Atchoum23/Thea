...
public struct ModelCapabilityView: View {
    ...
    @State private var selectedTaskType: ModelCapabilityRecord.CapabilityTaskType?
    ...
    private var taskTypeFilter: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ...
                ForEach(ModelCapabilityRecord.CapabilityTaskType.allCases, id: \.self) { taskType in
                    TaskTypeButton(
                        taskType: taskType,
...
private struct TaskTypeButton: View {
    let taskType: ModelCapabilityRecord.CapabilityTaskType?
    let isSelected: Bool
    let action: () -> Void
