    ...
    public actor PhaseOrchestrator {
        ...
        public enum OrchestratorError: Error, LocalizedError, Sendable {
            ...
            public var errorDescription: String? {
                switch self {
                case let .phaseNotFound(number):
                    "Phase \(number) not found in spec"
                case let .dependencyNotMet(dep):
                    "Dependency not met: \(dep)"
                case let .approvalRejected(reason):
        ...
        private func updateSpecWithCompletion(phase: PhaseDefinition) async throws {
            ...
            let specLocations = await [
                (getBasePath() as NSString).appendingPathComponent("Documentation/Architecture/THEA_MASTER_SPEC.md"),
                (getBasePath() as NSString).appendingPathComponent("Planning/THEA_SPECIFICATION.md"),
                (getBasePath() as NSString).appendingPathComponent("THEA_MASTER_SPEC.md")
            ]
