...
public final class PredictiveLifeEngine: ObservableObject {
    ...
    private func saveState() {
        ...
        do {
            let historyData = try JSONEncoder().encode(Array(predictionHistory.suffix(500)))
            UserDefaults.standard.set(historyData, forKey: predictionHistoryKey)
        } catch {
            logger.debug("Failed to persist prediction history: \(error.localizedDescription)")
        }
    ...
    private func loadState() {
        if let historyData = UserDefaults.standard.data(forKey: predictionHistoryKey) {
            do {
                ...
                logger.debug("Failed to load prediction history: \(error.localizedDescription)")
            }
        }
