...
public final class OpenRouterProvider: AIProvider, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "com.thea.v2", category: "OpenRouterProvider")
    ...
    public var supportedModels: [AIModel] {
        // OpenRouter supports most models from other providers
        AIModel.allKnownModels
    }

    public var capabilities: Set<ProviderCapability> {
        [.chat, .streaming, .vision, .functionCalling, .multimodal]
    }
    ...
    public func chat(
        ...
        request.setValue("Thea/2.0", forHTTPHeaderField: "HTTP-Referer")
        request.setValue("Thea AI Assistant", forHTTPHeaderField: "X-Title")
