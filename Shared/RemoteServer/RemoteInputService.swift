...
public class RemoteInputService: ObservableObject {
    ...
    public func handleRequest(_ request: InputRequest) async throws {
        ...
        case .mouseDrag(let fromX, let fromY, let toX, let toY, let button):
...
public enum RemoteInputError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .accessibilityNotEnabled: return "Accessibility permission is required for input control"
        case .notSupportedOnPlatform: return "Remote input is not supported on this platform"
        case .invalidKeyCode: return "Invalid key code provided"
        case .eventCreationFailed: return "Failed to create input event"
        }
    }
}
