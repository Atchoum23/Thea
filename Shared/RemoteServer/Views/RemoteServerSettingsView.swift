            ...
            if !server.connectedClients.isEmpty {
                Section {
                    ForEach(server.connectedClients) { client in
                        ClientRow(client: client, onDisconnect: {
                            Task {
    ...
    var body: some View {
        HStack {
            ...
            VStack(alignment: .leading) {
                ...
                if case .error(let message) = server.serverStatus {
                    Text(message)
                        .font(.caption)
...
private struct SecurityEventRow: View {
    ...
    private var eventIcon: String {
        switch event.type {
        case .serverStarted, .serverStopped: return "power"
        case .clientConnected: return "person.badge.plus"
        case .clientDisconnected: return "person.badge.minus"
        case .connectionRejected, .authenticationFailed: return "exclamationmark.shield"
        case .permissionDenied: return "lock.shield"
        case .rateLimitExceeded: return "speedometer"
        case .suspiciousActivity: return "exclamationmark.triangle"
        case .fileAccessBlocked, .commandBlocked: return "xmark.shield"
        case .serverError: return "exclamationmark.circle"
        }
    }

    private var eventColor: Color {
        switch event.type {
        case .serverStarted, .clientConnected: return .green
        case .serverStopped, .clientDisconnected: return .gray
        case .connectionRejected, .authenticationFailed, .permissionDenied: return .orange
        case .rateLimitExceeded, .suspiciousActivity, .fileAccessBlocked, .commandBlocked: return .red
