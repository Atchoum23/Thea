...
public class WorkWithAppsService: ObservableObject {
    ...
    public func execute(
        ...
        switch app.definition.bundleId {
        case "com.apple.finder":
            result = try await executeFinderAction(action, parameters: parameters)
        case "com.apple.Safari":
            result = try await executeSafariAction(action, parameters: parameters)
        case "com.apple.mail":
            result = try await executeMailAction(action, parameters: parameters)
        case "com.apple.Notes":
            result = try await executeNotesAction(action, parameters: parameters)
        case "com.apple.reminders":
            result = try await executeRemindersAction(action, parameters: parameters)
        case "com.apple.iCal":
            result = try await executeCalendarAction(action, parameters: parameters)
        case "com.apple.Terminal":
            result = try await executeTerminalAction(action, parameters: parameters)
        default:
            result = try await executeGenericAction(action, on: app, parameters: parameters)
        }
    ...
    private func executeCalendarAction(_ action: String, parameters: [String: Any]) async throws -> AppActionResult {
        ...
        case "createEvent":
            guard let title = parameters["title"] as? String,
                  let startDate = parameters["startDate"] as? Date else {
...
public enum WorkWithAppsError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .accessibilityNotEnabled:
            return "Accessibility permissions are required. Please enable in System Settings > Privacy & Security > Accessibility."
        case .appNotFound:
