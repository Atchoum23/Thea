...
struct PrivacySettingsView: View {
    ...
    @State private var privacyConfig = PrivacySettingsConfiguration.load()
    ...
    private var customPatternEditorSheet: some View {
        NavigationStack {
            Form {
    ...
    private func auditIcon(for type: PrivacyAuditEventType) -> String {
        switch type {
    ...
    private func auditColor(for type: PrivacyAuditEventType) -> Color {
        switch type {
    ...
    private func thirdPartyRow(name: String, description: String, privacyUrl: String) -> some View {
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                Text(name)
                    .font(.caption)
...
private struct PrivacySettingsConfiguration: Equatable, Codable {
    // Data Collection
...
private enum PrivacyRetentionPeriod: String, Codable {
    case sevenDays
    case thirtyDays
    case ninetyDays
    case oneYear
...
private enum PrivacyLockTimeout: String, Codable {
    case immediately
    case oneMinute
    case fiveMinutes
    case fifteenMinutes
...
private enum PrivacyBiometricType {
    case faceID
    case touchID
    case none
...
private enum PrivacyExportFormat: String, Codable {
    case json
    case csv
    case encrypted
}

private enum PrivacyAuditEventType: String, Codable {
    case dataAccess
    case dataExport
    case dataDelete
    case settingsChange
...
private struct PrivacyAuditLogEntry: Identifiable, Codable, Equatable {
    let id: UUID
