    ...
    var systemResourcesSection: some View {
        Section("System Resources") {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Toggle("Preload Default Model", isOn: $settingsManager.preloadDefaultModel)
                ...
                Text("Route simple queries to local models first for faster, private responses.")
                    .font(.caption)
                    .foregroundStyle(.tertiary)
            }
        }
...
extension LocalModelsSettingsView {
    ...
    func testOllamaConnection() {
        let ollamaURL = settingsManager.ollamaURL
        Task {
            do {
                // Test connection by hitting Ollama's API endpoint
                guard let url = URL(string: "\(ollamaURL)/api/tags") else {
                    print("❌ Invalid Ollama URL: \(ollamaURL)")
                    return
                }
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    if httpResponse.statusCode == 200 {
                        print("✅ Ollama connection successful to: \(ollamaURL)")
                    } else {
