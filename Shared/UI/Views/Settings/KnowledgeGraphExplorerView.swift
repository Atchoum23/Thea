...
struct KnowledgeGraphExplorerView: View {
    ...
    private var statisticsSection: some View {
        Section {
            HStack(spacing: 24) {
                statCard("Entities", value: "\(stats?.entityCount ?? 0)", icon: "circle.hexagongrid")
                statCard("Edges", value: "\(stats?.edgeCount ?? 0)", icon: "arrow.triangle.branch")
                statCard("Avg Connections", value: String(format: "%.1f", stats?.averageConnections ?? 0), icon: "point.3.connected.trianglepath.dotted")
            }
        } header: {
    ...
    private var typeBreakdownSection: some View {
        Section {
            if let distribution = stats?.typeDistribution, !distribution.isEmpty {
        ...
        Section {
            ...
            if !searchResults.isEmpty {
                ForEach(searchResults) { entity in
                    Button { selectEntity(entity) } label: {
                        entityRow(entity)
                    }
                    .buttonStyle(.plain)
                }
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    Image(systemName: icon(for: entity.type))
                        .foregroundStyle(color(for: entity.type))
                    Text(entity.name)
                        .font(.theaTitle3)
                    Spacer()
                ...
                if !entity.attributes.isEmpty {
                    ForEach(Array(entity.attributes), id: \.key) { key, value in
                        LabeledContent(key.capitalized, value: value)
                            .font(.theaCaption1)
                    }
                }

                LabeledContent("References", value: "\(entity.referenceCount)")
                    .font(.theaCaption1)
                LabeledContent("Last Updated", value: entity.lastUpdatedAt.formatted(.dateTime.month().day().hour().minute()))
                    .font(.theaCaption1)

                if !entityEdges.isEmpty {
                    Text("Relationships (\(entityEdges.count))")
                        .font(.theaCaption1)
                        .foregroundStyle(.secondary)
                        .padding(.top, 4)
                    ForEach(Array(entityEdges.prefix(10).enumerated()), id: \.offset) { _, edge in
                        HStack(spacing: 4) {
                            Text(edge.sourceID == entity.id ? edge.targetID : edge.sourceID)
                                .font(.theaCaption2)
                                .lineLimit(1)
    ...
    private func loadData() async {
        ...
        recentEntities = await graph.recentEntities(limit: 20)
        isLoading = false
    }
    ...
    private func selectEntity(_ entity: KGEntity) {
        ...
        Task {
            entityEdges = await PersonalKnowledgeGraph.shared.relationships(for: entity.id)
        }
    }

    // MARK: - Helpers

    private func icon(for type: KGEntityType) -> String {
        switch type {
        case .person: "person.fill"
        case .place: "mappin.circle.fill"
        case .habit: "repeat.circle.fill"
        case .goal: "target"
        case .healthMetric: "heart.fill"
        case .project: "folder.fill"
        case .event: "calendar"
        case .topic: "text.book.closed.fill"
        case .skill: "star.fill"
        case .preference: "slider.horizontal.3"
        }
    }

    private func color(for type: KGEntityType) -> Color {
        switch type {
        case .person: .blue
        case .place: .orange
        case .habit: .purple
        case .goal: .green
        case .healthMetric: .red
        case .project: .teal
        case .event: .indigo
        case .topic: .brown
        case .skill: .yellow
        case .preference: .cyan
        }
    }
}
