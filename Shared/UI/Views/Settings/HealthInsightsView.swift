...
struct HealthInsightsView: View {
        ...
        Section {
            if let report = pipeline.lastAnalysis {
                HStack(spacing: 20) {
                    ZStack {
                        Circle()
                            .stroke(scoreColor(report.overallScore).opacity(0.2), lineWidth: 10)
                        Circle()
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text(scoreLabel(report.overallScore))
                            .font(.theaTitle3)
                            .foregroundStyle(scoreColor(report.overallScore))
                        Text("Based on \(report.dataPoints.count) data point(s)")
                            .font(.theaCaption1)
                            .foregroundStyle(.secondary)
                        Text(report.date.formatted(.dateTime.month().day().hour().minute()))
                            .font(.theaCaption2)
                ...
                HStack {
                    ...
                    VStack(alignment: .leading) {
                        Text("No analysis yet")
                            .font(.theaBody)
                        Text("Run an analysis to see your health score")
                            .font(.theaCaption1)
                            .foregroundStyle(.secondary)
                    }
                    Spacer()
    ...
    // MARK: - Active Insights

    private var insightsSection: some View {
        Section {
            if pipeline.activeInsights.isEmpty {
                Text("No active insights. Run an analysis or check back later.")
                    .font(.theaCaption1)
                    .foregroundStyle(.secondary)
            } else {
                ForEach(pipeline.activeInsights) { insight in
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        if !insight.suggestion.isEmpty {
                            Label(insight.suggestion, systemImage: "lightbulb.fill")
                                .font(.theaCaption2)
                        ...
                        HStack {
                            Text(insight.category.rawValue.capitalized)
                                ...
                                .padding(.vertical, 2)
                                .background(severityColor(insight.severity).opacity(0.12), in: Capsule())
                            Spacer()
                        }
                    }
                    .padding(10)
            ...
            HStack {
                Text("Active Insights (\(pipeline.activeInsights.count))")
                    .font(.theaHeadline)
                Spacer()
                if !pipeline.activeInsights.isEmpty {
                    Button("Clear All") {
    ...
    private var analysisStatusSection: some View {
        Section {
            HStack(spacing: 16) {
                LabeledContent("Coaching") {
                    Text(pipeline.isEnabled ? "Enabled" : "Disabled")
                        .foregroundStyle(pipeline.isEnabled ? .green : .red)
                }
                LabeledContent("Cooldown") {
                    Text("\(pipeline.analysisCooldownHours)h")
                }
                LabeledContent("Last Run") {
                    if let date = pipeline.lastAnalysisDate {
    ...
    private func severityIcon(_ severity: CoachingSeverity) -> String {
        switch severity {
