  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 4) {
        Label(
          "\(sourceInfo.estimate.conversationCount) conversations",
          systemImage: "bubble.left.and.bubble.right")
        Label("\(sourceInfo.estimate.projectCount) projects", systemImage: "folder")
        Label(
          ByteCountFormatter.string(
            fromByteCount: sourceInfo.estimate.totalSizeBytes, countStyle: .file),
  ...
  var body: some View {
    NavigationStack {
      VStack(spacing: 30) {
        ...
        } else if migrationEngine.completedMigrations.contains(where: {
          $0.source == source.sourceName
        }) {
          // Complete
          VStack(spacing: 16) {
            Image(systemName: "checkmark.circle.fill")
