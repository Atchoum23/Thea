  ...
  var body: some View {
    Form {
      ...
      Section("Generation Defaults") {
        Stepper(
          "Max Tokens: \(config.defaultMaxTokens)", value: $config.defaultMaxTokens,
      ...
      Section("Model Defaults") {
        ...
        Stepper(
          "Embedding Dimensions: \(config.embeddingDimensions)", value: $config.embeddingDimensions,
          in: 256...4096, step: 256)
      }

      Section("Request Settings") {
        Stepper(
          "Timeout (seconds): \(Int(config.requestTimeoutSeconds))",
          value: $config.requestTimeoutSeconds, in: 10...300, step: 10)
        Stepper("Max Retries: \(config.maxRetries)", value: $config.maxRetries, in: 0...10)
        Stepper(
          "Retry Delay (seconds): \(Int(config.retryDelaySeconds))",
          value: $config.retryDelaySeconds, in: 0...10, step: 1)
      }
  ...
  var body: some View {
    Form {
      ...
      Section("Speech Recognition") {
        ...
        Stepper(
          "Audio Buffer Size: \(config.audioBufferSize)", value: $config.audioBufferSize,
          in: 256...4096, step: 256)
      }
  ...
  var body: some View {
    Form {
      ...
      Section("File Limits") {
        ...
        Stepper(
          "Batch Size: \(config.indexingBatchSize)", value: $config.indexingBatchSize, in: 10...500,
          step: 10)
      }

      Section("Embedding") {
        Stepper(
          "Embedding Dimension: \(config.embeddingDimension)", value: $config.embeddingDimension,
          in: 128...1536, step: 128)
      }

      Section("Search") {
        Stepper(
          "Default Top K: \(config.defaultSearchTopK)", value: $config.defaultSearchTopK,
          in: 5...50, step: 5)
        Stepper(
          "Full-Text Top K: \(config.fullTextSearchTopK)", value: $config.fullTextSearchTopK,
          in: 5...50, step: 5)
      }
...
struct MemoryConfigurationView: View {
  ...
  var body: some View {
    Form {
      Section("Capacity") {
        Stepper(
          "Short-Term: \(config.shortTermCapacity)", value: $config.shortTermCapacity, in: 5...100,
          step: 5)
        Stepper(
          "Long-Term Max: \(config.longTermMaxItems)", value: $config.longTermMaxItems,
          in: 1000...50000, step: 1000)
        Stepper(
          "Episodic Max: \(config.episodicMaxItems)", value: $config.episodicMaxItems,
          in: 1000...20000, step: 1000)
        Stepper(
          "Semantic Max: \(config.semanticMaxItems)", value: $config.semanticMaxItems,
          in: 1000...20000, step: 1000)
        Stepper(
          "Procedural Max: \(config.proceduralMaxItems)", value: $config.proceduralMaxItems,
      ...
      Section("Retrieval") {
        Stepper(
          "Default Limit: \(config.defaultRetrievalLimit)", value: $config.defaultRetrievalLimit,
          in: 5...50, step: 5)
  ...
  var body: some View {
    Form {
      Section("Task Execution") {
        ...
        Stepper(
          "Base Task Duration: \(Int(config.baseTaskDurationSeconds))s",
          value: $config.baseTaskDurationSeconds, in: 10...120, step: 10)
      }

      Section("Sub-Agents") {
        Stepper(
          "Max Concurrent: \(config.maxConcurrentAgents)", value: $config.maxConcurrentAgents,
          in: 1...20)
        Stepper(
          "Timeout: \(Int(config.agentTimeoutSeconds))s", value: $config.agentTimeoutSeconds,
          in: 60...600, step: 60)
      }

      Section("Reasoning") {
        Stepper(
          "Chain of Thought Steps: \(config.chainOfThoughtSteps)",
          value: $config.chainOfThoughtSteps, in: 2...10)
        Stepper(
          "Max Decomposition: \(config.maxDecompositionSteps)",
          value: $config.maxDecompositionSteps, in: 5...20)
...
struct LocalModelConfigurationView: View {
  ...
  var body: some View {
    Form {
      ...
      Section("Defaults") {
        Stepper(
          "Context Tokens: \(config.defaultContextTokens)", value: $config.defaultContextTokens,
          in: 1024...32768, step: 1024)
        Stepper(
          "Max Output Tokens: \(config.defaultMaxOutputTokens)",
          value: $config.defaultMaxOutputTokens, in: 256...8192, step: 256)
  ...
  var body: some View {
    Form {
      Section("THEA") {
        LabeledContent(
          "Version",
          value: "\(AppConfiguration.AppInfo.version) (\(AppConfiguration.AppInfo.buildType))")
  ...
  var body: some View {
    Form {
      Section("Test Models for API Key Validation") {
        Text(
          "These models are used when validating API keys. Using smaller, faster models reduces validation time and cost."
        )
        .font(.caption)
        .foregroundStyle(.secondary)
      }
