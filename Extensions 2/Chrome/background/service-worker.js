...
async function handleMessage(message, sender) {
  switch (message.type) {
    ...
    case 'lifeMonitorData':
      return await handleLifeMonitorData(message.data);

    case 'getLifeMonitorSettings':
      return { success: true, data: getLifeMonitorSettings() };

    case 'updateLifeMonitorSettings':
      return await handleUpdateLifeMonitorSettings(message.data);
...
const defaultLifeMonitorSettings = {
  ...
  sensitiveUrlPatterns: [
    /banking|bank\./i,
    /paypal|venmo|stripe/i,
    /healthcare|medical|health\./i,
    /password|login|signin|auth/i,
    /\.gov\//i,
    /mail\.google|outlook\.live|protonmail/i
  ],
...
let lifeMonitorSyncTimer = null;

/**
 * Get current life monitor settings
 */
function getLifeMonitorSettings() {
  return {
...
async function handleUpdateLifeMonitorSettings(newSettings) {
  // Only allow safe settings to be updated
  const allowedKeys = [
  ...
  await chrome.storage.local.set({ theaLifeMonitorSettings: lifeMonitorSettings });

  // Notify all tabs of settings change
  broadcastToTabs({ type: 'lifeMonitorSettingsChanged', data: getLifeMonitorSettings() });
 ...
 */
async function handleLifeMonitorData(data) {
  ...
  // Validate and process based on event type
  switch (eventType) {
    case 'pageVisit':
      return await processPageVisit(payload);

    case 'readingSession':
      return await processReadingSession(payload);

    case 'linkClick':
      return await processLinkClick(payload);

    case 'textSelection':
      return await processTextSelection(payload);

    case 'formInteraction':
      return await processFormInteraction(payload);

    default:
      console.warn('Unknown life monitor event type:', eventType);
      return { success: false, error: 'Unknown event type' };
  }
}

/**
 * Process page visit event
 */
async function processPageVisit(payload) {
  if (!lifeMonitorSettings.capturePageContent) {
    // Just track URL without content
    payload.content = null;
    payload.textContent = null;
  }

  // Truncate content if needed
  if (payload.textContent && payload.textContent.length > lifeMonitorSettings.maxContentLength) {
    payload.textContent = payload.textContent.substring(0, lifeMonitorSettings.maxContentLength) + '...[truncated]';
  }
  ...
  if (pendingLifeMonitorEvents.length >= lifeMonitorSettings.batchSize) {
...
/**
 * Process reading session event
 */
async function processReadingSession(payload) {
  if (!lifeMonitorSettings.trackReadingBehavior) {
    return { success: false, error: 'Reading behavior tracking is disabled' };
  }

  // Only record sessions with meaningful reading time
  if (payload.timeOnPageMs < lifeMonitorSettings.minReadingTimeMs) {
    return { success: true, ignored: true, reason: 'Reading time too short' };
  }
 ...
 */
async function processLinkClick(payload) {
  if (!lifeMonitorSettings.trackNavigationHistory) {
    return { success: false, error: 'Navigation tracking is disabled' };
  }
...
/**
 * Process text selection event
 */
async function processTextSelection(payload) {
  if (!lifeMonitorSettings.trackReadingBehavior) {
    return { success: false, error: 'Reading behavior tracking is disabled' };
  }

  // Only record meaningful selections
  if (!payload.selectedText || payload.selectedText.length < 5) {
  ...
  pendingLifeMonitorEvents.push({
    ...
    data: {
      url: payload.url,
      selectedText: payload.selectedText.substring(0, 500), // Limit selection length
...
/**
 * Process form interaction event (non-sensitive forms only)
 */
async function processFormInteraction(payload) {
  // This is primarily for search queries and non-sensitive forms
  pendingLifeMonitorEvents.push({
...
/**
 * Sync life monitor data to THEA app via WebSocket
 */
async function syncLifeMonitorData() {
  ...
  try {
    // Try WebSocket connection to THEA app
    const ws = new WebSocket('ws://localhost:9876/browser');
    ...
    return new Promise((resolve) => {
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          if (response.success) {
            resolve({ success: true, synced: eventsToSync.length });
          } else {
            // Put events back in queue on failure
            pendingLifeMonitorEvents.unshift(...eventsToSync);
...
// Load life monitor settings on startup
chrome.storage.local.get('theaLifeMonitorSettings').then(stored => {
  if (stored.theaLifeMonitorSettings) {
    lifeMonitorSettings = { ...defaultLifeMonitorSettings, ...stored.theaLifeMonitorSettings };
  }
});

// Set up periodic sync for life monitor data
chrome.alarms.create('syncLifeMonitor', { periodInMinutes: 0.5 }); // Every 30 seconds
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  switch (alarm.name) {
    ...
    case 'syncLifeMonitor':
      // Sync pending life monitor events to THEA app
